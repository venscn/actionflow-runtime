...
`ProcessedEventStore` is an optional StateStore extension type for future processed event id tracking:
...
- No MemoryStateStore or FileStateStore implementation exists yet.
...
- ProcessedEventStore exists as an optional interface and support guard only; no store implementation, runtime integration, exactly-once behavior, or durable event recovery is implemented.
